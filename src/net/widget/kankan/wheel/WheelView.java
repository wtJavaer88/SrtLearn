...
public class WheelView extends View
{
    ...
    private float perHanziSize = 72;
    // 格式化文本长度, 以汉字作为标准,默认不格式化
    private float FORMAT_TEXT_LENGTH = 0;
    // Listeners
     ...
     * @return
     */
    public WheelView setFormatTextLength(float length)
    {
        FORMAT_TEXT_LENGTH = length;
        return this;
    }
    ...
    private boolean addViewItem(int index, boolean first)
    {
        ...
        if (FORMAT_TEXT_LENGTH > 0)
        {
            if (view instanceof TextView)
            {
                ...
                perHanziSize = tv.getPaint().measureText("啊");
                tv.setText(getSuiteTExt(tv, text));
            }
        }
        if (view != null)
        {
    ...
    /**
     * 获取当前字体下,不满足FORMAT_TEXT_LENGTH的文本的填充结果
     * 
     * @param tv
     * @param text
     * @return
     */
    private String getSuiteTExt(TextView tv, String text)
    {
        float orginalWidth = tv.getPaint().measureText(text);
        int delta = (int) Math
                .ceil((perHanziSize * FORMAT_TEXT_LENGTH - orginalWidth)
                        / (perHanziSize / 2));
        final String blank = getBlank((int) Math.ceil(delta / 2));
