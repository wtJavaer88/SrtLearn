    ...
    public static Set<Topic> getCETTopic(int srtId)
    {
        ...
        try
        {
            ...
            if (topicCursor != null && topicCursor.getCount() > 0)
            {
                ...
                {
                    String real_word = topicCursor.getString(topicCursor
                            .getColumnIndex("real_word"));
                    int topic_id = topicCursor.getInt(topicCursor
                            .getColumnIndex("topic_id"));
                    String sql = "select * from topic_resource res,dictionary dict,books"
                            + " where res.topic=dict.topic_id and books.id=res.book_id and topic_id ="
                            + topic_id + " order by book_id desc";
                    ...
                    while (!c.isAfterLast())
                    {
                        ...
                        topic.setTopic_id(""
                                + c.getInt(c.getColumnIndex("topic_id")));
                        topic.setTopic_word(real_word);
                        topic.setMean_cn(c.getString(c
                                .getColumnIndex("mean_cn")));
                        topic.setTopic_base_word(c.getString(c
                                .getColumnIndex("topic_word")));
