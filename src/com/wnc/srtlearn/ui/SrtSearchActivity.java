...
public class SrtSearchActivity extends BaseVerActivity implements
        OnClickListener, UncaughtExceptionHandler
{
    ...
    private void initData()
    {
        ...
        if (intent != null
                && BasicStringUtil.isNotNullString(intent
                        .getStringExtra("dialog")))
        {
        ...
        for (String word : PatternUtil.getPatternStrings(dialog.toLowerCase(),
                "\\w+"))
        {
    ...
    private void initView()
    {
        ...
        ((Button) findViewById(R.id.btn_srtsearch_clear))
            ...
            public void afterTextChanged(final Editable s)
            {
                ...
                mHandler.post(new Runnable()
                {
                    ...
                    public void run()
                    {
                        ...
                        {
                            findViewById(R.id.btn_srtsearch_clear)
    ...
    public void onClick(View v)
    {
        ...
        case R.id.btn_srtsearch:
            ...
            if (BasicStringUtil.isNullString(keyword)
            ...
            {
                List<SearchSrtInfo> searchResult = SrtInfoDao.searchByLan(true,
                        keyword);
            ...
            break;
        case R.id.btn_srtsearch_clear:
            CharSequence text = act.getText();
            String content = text.toString();
            if (content.length() > 0)
            {
                this.act.setText(content.substring(0, content.length() - 1));
    ...
    {
        SimpleAdapter adapter = new SimpleAdapter(this, getData(searchResult),
                R.layout.lv_item_srtsearch, new String[]
                { "eng", "chs" }, new int[]
                { R.id.tvEng, R.id.evChs });
    ...
    public void showDialog(HashMap map)
    {
        ...
        final TextView tvEp = (TextView) dialog
                .findViewById(R.id.tvEpidoseInfo);
        String season = PatternUtil.getFirstPatternGroup(epStr, "(.*?)/");
        String eposide = PatternUtil.getFirstPatternGroup(epStr, "/(.*+)");
        tvEp.setText(season + "\n" + eposide);

        ((ImageButton) dialog.findViewById(R.id.imgbtn_ToContext))
                .setOnClickListener(new OnClickListener()
                {
                    ...
                    {
                        ToastUtil
                                .showShortToast(getApplicationContext(), "上下文");
                        startActivity(new Intent()
                                .setClass(getApplicationContext(),
                                        SrtActivity.class)
                                .putExtra("srtFilePath", ssrt.getSrtFile())
                                .putExtra("seektime",
                                        ssrt.getFromTime().toString()));
                    }
                });

        final String voicePath = SrtMediaUtil.getVoicePath(season, eposide,
                ssrt.getFromTime().toString().replace(":", ""));
        ImageButton imgbtVoice = (ImageButton) dialog
                .findViewById(R.id.imgbtn_PlayVoice);
        ...
        // ssrt.getToTime());
        ((TextView) dialog.findViewById(R.id.tvTimeLine)).setText(ssrt
    ...
    private void show(HashMap map)
    {
        ...
        tv.setText("("
                + index
                + ") \n"
                + TextFormatUtil.getFileNameNoExtend(ssrt.getSrtFile())
                        .replace("/", "\n")
                + "\n"
                + SrtTextHelper.concatTimeline(ssrt.getFromTime(),
                        ssrt.getToTime())); // 内容
