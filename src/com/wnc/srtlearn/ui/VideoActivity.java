...
public class VideoActivity extends Activity implements OnClickListener,
        ...
        CtrlableDoubleClickGestureDetectorListener, Favoritable
{
                ...
                try
                {
                    if (currentPosition > 0)
                    {
                        if (mediaPlayer != null)
                        {
                            ...
                            String path = SrtMediaUtil.getVideoFile(
                                    MyAppParams.VIDEO_FOLDER, videoSeries,
                                    videoEpisode);
        ...
        new Thread(new Runnable()
        {
            ...
            public void run()
            {
                ...
                while (++i <= 15)
                {
                    ...
                    if (allSrtInfos != null
                            && allSrtInfos.size() > srtInfos.size())
                    {
    ...
    private void initHoldPlay()
    {
        ...
        String path = SrtMediaUtil.getVideoFile(MyAppParams.VIDEO_FOLDER,
        ...
        mediaPlayer.setOnPreparedListener(new OnPreparedListener()
        {
            ...
            public void onPrepared(MediaPlayer mp)
            {
                ...
                ((TextView) findViewById(R.id.totaltime)).setText(SrtTextHelper
                        .timeToText(seekBar.getMax()));
                mediaPlayer.seekTo(currentPosition);
                videoPlayListen();
            }
    ...
    public List<SrtInfo> getCurrentPlaySrtInfos()
    {
        try
        {
            if (isCusReplay())
            {
                return DataHolder.getSrtInfos(curIndex,
                        curIndex + Math.max(0, replayInfo.getSrtcounts() - 1));
            }
            else
            {
    ...
    {
        final int defaultSrtCount = 2;
        final int defaultPreLoad = 0;
        final int defaultAfterLoad = 0;
        ...
        if (replayInfo.getPreLoad() > 0)
        {
            mView2.setNum(replayInfo.getPreLoad());
        }
        else
        {
            mView2.setNum(defaultPreLoad);
        }
        ...
        if (replayInfo.getAfterLoad() > 0)
        {
            mView3.setNum(replayInfo.getAfterLoad());
        }
        else
        {
            mView3.setNum(defaultAfterLoad);
        }
        ...
        TextView btnReset = (TextView) replaySettingDialog
                .findViewById(R.id.srt_replay_set_dialg_reset);
        ...
        btnReset.setOnClickListener(new OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                replayInfo.setSrtcounts(defaultSrtCount);
                replayInfo.setPreLoad(defaultPreLoad);
                replayInfo.setAfterLoad(defaultAfterLoad);
                mView1.setNum(defaultSrtCount);
                mView2.setNum(defaultPreLoad);
                mView3.setNum(defaultAfterLoad);
            }
    ...
    {
        ((TextView) findViewById(R.id.curtime)).setText(SrtTextHelper
                .timeToText(position));
    }
    ...
    {
        return seek >= TimeHelper.getTime(curSrt2.getFromTime())
                && seek < TimeHelper.getTime(curSrt2.getToTime());
    }
    ...
    public void onConfigurationChanged(Configuration newConfig)
    {
        ...
        if (isLandscape())
        {
            ...
            ((RelativeLayout) videoView.getParent())
        ...
        else if (this.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT)
        {
            ...
            ((RelativeLayout) videoView.getParent())
                    .setLayoutParams(new LinearLayout.LayoutParams(
                            LinearLayout.LayoutParams.MATCH_PARENT,
                            (int) (0.4 * Math.max(MyAppParams.getScreenWidth(),
