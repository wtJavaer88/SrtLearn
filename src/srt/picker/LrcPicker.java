...
public class LrcPicker implements Picker {
	...
	public static void main(String[] args) {
		List<SrtInfo> srtInfos = PickerFactory.getPicker("10.ai ei ui.lrc").getSrtInfos();
	...
	private TimeInfo parseTimeInfo(String timeStr) {
		int hour = 0;
		int minute = BasicNumberUtil.getNumber(PatternUtil.getFirstPattern(timeStr, "\\d{2}:").replace(":", ""));
		int second = BasicNumberUtil.getNumber(PatternUtil.getFirstPattern(timeStr, "\\d{2}\\.").replace(".", ""));
		int millSecond = 10 * BasicNumberUtil.getNumber(PatternUtil.getLastPattern(timeStr, "\\d{2}"));
	...
	public List<SrtInfo> getSrtInfos(int start, int end) {
		...
		for (int i = start; i < end && i < segments.size(); i++) {
			...
			fromTime = parseTimeInfo(PatternUtil.getFirstPattern(str, "\\d{2}:\\d{2}\\.\\d{2}"));
			if (index > 0) {
				srtInfos.get(index - 1).setToTime(fromTime);
			}
			chs = PatternUtil.getFirstPattern(str, "\\].*?\\\\N").replace("]", "").replace("\\N", "");
