...
{// 两个音频间的播放延迟
	...
	private long palyVoice() throws SrtException
	{
		...
		Queue<String> srtVoicesWithBg = SrtMediaUtil.getSrtVoicesInRange(DataHolder.getFileKey(), currentSrtInfo.getFromTime().toString(), currentSrtInfo.getToTime().toString());
		long curSrtduration = TimeHelper.getTime(currentSrtInfo.getToTime()) - TimeHelper.getTime(currentSrtInfo.getFromTime());
	...
	private long getVoiceDuration(final SrtInfo currentSrtInfo, Queue<String> srtVoicesWithBg) throws SrtException
	{
		...
		try
		{
			nextSrtInfo = DataHolder.getSrtInfoByIndex(DataHolder.getCurrentSrtIndex() + 1);

			// 如果第二段是背景声音,则播放并累加时间
			if (!secondPath.contains(nextSrtInfo.getFromTime().toString().replace(":", "")))
			{
				final long l = TimeHelper.getTime(nextSrtInfo.getFromTime()) - TimeHelper.getTime(currentSrtInfo.getToTime());
				addTime = l;
			}
			else
			{
